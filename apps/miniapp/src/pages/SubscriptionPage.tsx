...
{(showSubscription) && (<>
    <div data-screen-label="Subscription" style={{"position": "absolute", "inset": "0", "display": "flex", "flexDirection": "column"}}>
      ...
      <div style={{"flex": "1", "overflowY": "auto", "padding": "8px 22px 30px"}}>
        ...
        <div style={{"fontSize": "15px", "fontWeight": "600", "color": "#e9e2e8", "margin": "22px 0 14px"}}>Payment method</div>
        {(payMethods||[]).map((m, _kp: number) => (<React.Fragment key={_kp}>
          <div data-press="" onClick={m.onSelect} style={m.cardStyle}>
        ...
        {(subAutoRenew) && (<div style={{"display": "flex", "alignItems": "flex-start", "gap": "9px", "marginTop": "2px", "padding": "12px 14px", "borderRadius": "12px", "background": "rgba(247,178,59,.06)", "border": "1px solid rgba(247,178,59,.22)"}}>
          ...
          <div style={{"fontSize": "13px", "color": "#c9b48a", "lineHeight": "1.45"}}>Renews automatically every month. Cancel anytime in Telegram → Settings → My Stars.</div>
        </div>)}
